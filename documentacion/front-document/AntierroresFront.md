...
### EF-33 — `ngOnInit` en una página de tab corre UNA SOLA VEZ por sesión: los datos quedan congelados
- Qué pasó: un admin vencía o extendía la fecha de una oferta/cupón y en la vista cliente **el cambio no aparecía**. Solo se veía después de un F5 completo — y en la app Android compilada no hay F5, así que ahí quedaba congelado hasta reiniciar la app.
- Causa: `home` y `ofertas` cargaban sus datos en `ngOnInit`. **Ionic cachea las páginas de los tabs en lugar de destruirlas** (se quedan montadas en el `ion-router-outlet` para que volver a un tab sea instantáneo y conserve el scroll). Al no destruirse el componente, `ngOnInit` **no se vuelve a ejecutar nunca**: corre una vez al montar y listo. Cambiar de tab y volver no dispara nada, así que la lista mostrada era la del primer momento de la sesión.
- Descarte importante: **no era caché ni backend**. El `OfertaRepository` filtra por fecha (`fecha_fin >= hoy`) en cada consulta, así que un request nuevo ya traía la oferta extendida; y ni `oferta.service.ts` ni `cupon.service.ts` tienen `shareReplay` ni ningún almacenamiento. El request correcto simplemente nunca se hacía de nuevo.
- Solución: mover la carga a **`ionViewWillEnter`**, que sí se dispara en cada entrada al tab (y también en la primera, por eso hay que **mover** la llamada, no dejarla además en `ngOnInit` — si no, doble request en el arranque). En `ngOnInit` solo quedó lo que de verdad corre una vez (la suscripción a query params).
- Matiz de granularidad: no siempre conviene recargar *todo*. En `home`, `cargarVitrina()` (productos) se dejó en `ngOnInit` y se extrajo `cargarPromos()` (ofertas + cupones) para `ionViewWillEnter` — recargar los productos haría parpadear el skeleton del menú en cada entrada al tab, y el menú no caduca por fecha como sí lo hacen las promos. Refrescá lo que caduca, no la pantalla entera.
- Límite conocido (ver también la nota de `mi-cuenta`): `ionViewWillEnter` **tampoco se dispara** al volver de una ruta que está FUERA de `/tabs` (login/register), porque ahí Ionic cachea el shell entero de tabs. Para ese cruce hace falta otro mecanismo (`ionViewDidEnter` del shell, o resolverlo en el handler de navegación).
- Regla: en una página que vive dentro de un tab, `ngOnInit` sirve para setup de una sola vez (suscripciones, query params), **nunca** para cargar datos que pueden cambiar del lado del servidor. Esos van en `ionViewWillEnter`. Si al revisar una pantalla ves `ngOnInit` llamando a un `cargarX()` que pega al backend, es un bug de dato viejo esperando a que alguien lo note.
- Fecha: 2026-08-16
...
### EF-21 — `align-items:stretch` iguala el ALTO del contenedor, no el de la última card de una columna multi-card
- Qué pasó: en Reseñas, la fila `.res-row` (izquierda = 2 cards apiladas: "Distribución" + "Filtrar por estrellas"; derecha = card "Filtrar" con `[fill]`). Con `align-items:stretch` la medición decía alturas iguales (los DIV de columna medían lo mismo), pero visualmente "Filtrar" (derecha) se veía un pelín más alto: el borde inferior de la card de estrellas quedaba por encima del de Filtrar.
- Causa: `align-items:stretch` estira el **DIV de la columna izquierda** al alto de la fila (que lo manda el lado más alto), pero sus dos cards conservan su alto natural y quedan pegadas ARRIBA → sobra un hueco al fondo del DIV, debajo de la última card. Ese hueco es transparente, así que el fondo de la card de estrellas no llega abajo. Medir el DIV daba "igual"; medir el `getBoundingClientRect().bottom` de la **card** (no del DIV) revelaba el desfase (~5px).
- Solución: hacer que una card de la columna CREZCA para absorber el hueco y empujar la última al fondo. Se le dio `flex:1 1 auto` a la card "Distribución" + su `[fill]="true"` (para que su `.section-card` interno también llene con `height:100%` y el fondo se extienda). Así "Filtrar por estrellas" baja hasta el fondo y los bordes inferiores de ambas columnas coinciden exacto (`difBottom:0`).
- Regla: en una fila flex con `align-items:stretch`, una columna que a su vez apila varias cards NO reparte sola el alto extra — hay que darle `flex-grow` (+ `[fill]` en `admin-section-card`) a una de sus cards para que ocupe el hueco. Y al verificar simetría de alturas, medí el `bottom` de las CARDS visibles, no el del DIV contenedor (el DIV puede medir "igual" y aun así verse desparejo).
- Fecha: 2026-08-08
