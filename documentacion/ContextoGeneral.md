...
- Próximos: integración de cupones en el checkout, descuento automático de insumos al confirmar un pedido, eliminar sucursales (hoy el CRUD es solo crear/editar), guard de rol real en Angular para `/admin`, resto de módulos del admin (analíticas, notificaciones, reseñas, configuración) vía `api-integration-helper`, decidir si se elimina del todo el endpoint backend `home-config` (huérfano desde 2026-07-25), "Continuar con Google" (fast-follow). Detalle en `back-document/HiloActualBack.md` y `front-document/HiloActualFront.md`.
