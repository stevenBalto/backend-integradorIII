...
- **Panel admin (resto): base visual lista.** Shell con sidebar + módulos en `frotend-integradorIII/src/app/admin/` (incluye `superadmin/` aparte, panel independiente). Menú, Ofertas y cupones (con KPIs clicables + buscador funcional), Inventario, Pedidos, **Inicio** (curación del Home: destacados/popular/nuevo, oferta "hero", preview de cupones), **Clientes** (analítica de compra) y **Usuarios y roles** (CRUD real: crear/editar/eliminar staff, asignación de módulos por usuario, ver nota abajo) ya conectados a la API real. Dashboard, Analíticas, Notificaciones, Reseñas, Configuración (resto) siguen maquetado estático. El atajo temporal `admin`/`123` en el login YA NO EXISTE — el acceso ahora depende del rol real devuelto por el backend (login unificado, redirige a `/admin`, `/superadmin/panel` o `/tabs/home` según `tipo`/`rol`).
