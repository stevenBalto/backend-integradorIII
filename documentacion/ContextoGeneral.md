...
- **Diálogos de confirmación propios (2026-08-08): FUNCIONAL.** `ConfirmService` + `ConfirmDialogComponent` reemplazan a `window.confirm`/`window.alert` en las 8 llamadas del proyecto (inventario, menú, ofertas, cupones, pedidos, carrito). Centrados, con estilo de marca y tono `peligro` para acciones destructivas. **De paso arregla el modo extendido de Pedidos**: los diálogos nativos sacan a Chrome del fullscreen, ver `AntierroresFront.md` **EF-20**. Regla nueva: no usar `window.confirm/alert/prompt` en esta app.
- **Login con Google en app nativa (2026-08-08): NO funciona tal como está — investigado, sin implementar.** El flujo actual es OAuth web (`window.location.href` + `origen = window.location.origin`) y en un build de Capacitor se rompe por tres motivos: (1) `window.location.origin` pasa a ser `https://localhost` / `capacitor://localhost`, que no es una URL a la que Google pueda volver; (2) **Google bloquea OAuth dentro de WebViews embebidos** (`disallowed_useragent`), es política suya; (3) volver a la app requiere un esquema/deep link registrado en el SO. Las dos salidas son navegador del sistema + deep link (RFC 8252, reutiliza casi todo el backend actual; `@capacitor/app` ya está instalado, faltaría `@capacitor/browser`) o un plugin nativo de Google Sign-In (mejor UX, pero pide endpoint nuevo en el backend). **Bloqueante previo a todo esto**: el backend tiene que estar en un hosting público con HTTPS (hoy `127.0.0.1:8000`), el `capacitor.config.ts` sigue con el `appId` de ejemplo `io.ionic.starter`, y no existen las carpetas `android/`/`ios/`. Requiere aprobación del usuario por la regla de no agregar paquetes npm.
